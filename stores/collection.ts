...
import type { UserCard } from '~/types/collection';
...
export const useCollectionStore = defineStore(
  ...
  () => {
    ...
    const fetchUserCards = async () => {
      ...
      try {
        const { data, error: fetchError } = (await useAPI('user/cards', {
          method: 'GET',
          default: () => [],
        })) as UseApiResponse<UserCard[]>;

        if (fetchError.value) {
          error.value =
            fetchError.value.data?.message || 'Erreur lors du chargement de la collection';
          return;
        }
    ...
    const addCardToCollection = async (
      ...
      try {
        const { error: fetchError } = (await useAPI('user/cards', {
          method: 'POST',
          body: {
            cardId,
            quantity,
